...
const Cardcolors = ({name,secondname,units,time, month, monthshow ,value}) => {
  ...
  return (
    <View style={[{height:heightValue(7.8),width:widthValue(1.1),},marginPosition(0,0,0,'5%'),darkMode?styles.bgWhite:styles.bgdashblue,radius(10)]}>
      <View style={[styles.row]}>
        ...
        <View style={[styles.column,{width:widthValue(3.4),height:heightValue(7.3),justifyContent:'center',alignItems:'flex-end'},marginPosition(0,0,0,'5%')]}>
        <View style={[styles.row,{height:heightValue}]}>
          {monthshow ? 
          <Text style={[darkMode?styles.black:styles.white,marginPosition(0,0,'2%','3%'),fontSize(10)]}>{time}</Text>:
        ...
        <Text style={[darkMode?styles.black:styles.white,fontSize(13),{marginLeft:setwidthvalue(month)}]}>{units}</Text>
